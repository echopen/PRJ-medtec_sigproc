...
class Notification(models.Model):
    ...
    _PROCESSED_RUN_TEMPLATE = u'<a href="/{0}/">{1}</a> your run terminate with success: <a href="/feeds/{2}/">{3}</a>'
    ...
    run = models.ForeignKey('leaderboard.Algorithm', null=True, blank=True)
	...
	elif self.notification_type == self.PROCESSED_RUN:
            return self._PROCESSED_RUN_TEMPLATE.format(
                escape(self.from_user.username),
                escape(self.from_user.profile.get_screen_name()),
                self.feed.pk,
                escape(self.get_summary(self.feed.post))
